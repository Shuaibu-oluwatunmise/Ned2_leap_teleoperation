...
class HandReceiver(Node):
    def __init__(self):
        ...
        # Add gesture publishers
        self.left_gesture_pub = self.create_publisher(String, '/hand_tracking/left_gesture', 10)
        self.right_gesture_pub = self.create_publisher(String, '/hand_tracking/right_gesture', 10)
    ...
    def process_hand_data(self, json_data: str):
        ...
        try:
            ...
            # DEBUG: Check gesture data
            if data.get('hands'):
                gestures = []
                for hand in data['hands']:
                    gesture = hand.get('gesture', 'MISSING')
                    hand_type = hand.get('type', 'unknown')
                    gestures.append(f"{hand_type}:{gesture}")
                
                if any('MISSING' not in g and 'none' not in g for g in gestures):
    ...
    def publish_hand_data(self, hand: Dict, timestamp: float):
        ...
        # Gesture message
        gesture_msg = String()
        gesture_msg.data = hand.get('gesture', 'none')
        
        # DEBUG: Log confirmed gestures
        gesture_data = hand.get('gesture', 'none')
        if gesture_data.endswith('_confirmed'):
            print(f"DEBUG: Publishing {hand_type} gesture: {gesture_data}")
        
        if hand_type == 'left':
            self.left_palm_pub.publish(pose_msg)
            self.left_grab_pub.publish(grab_msg)
            self.left_gesture_pub.publish(gesture_msg)
        elif hand_type == 'right':
            self.right_palm_pub.publish(pose_msg)
            self.right_grab_pub.publish(grab_msg)
            self.right_gesture_pub.publish(gesture_msg)
        
        if self.packet_count % 60 == 0:
            self.get_logger().info(
                f"{hand_type.title()} hand: pos=({position[0]:.1f}, {position[1]:.1f}, {position[2]:.1f}) "
                f"grab={hand.get('grab_strength', 0.0):.2f} gesture={gesture_data}"
            )
...
""")
    ...
    try:
        ...
        print("   - /hand_tracking/left_gesture")
        print("   - /hand_tracking/right_gesture")
        print()
